...
CREATE TABLE PERSONA.USUARIO(
    ...
    CONSTRAINT CK_CLAVE_ACCESO CHECK (LEN(CLAVE_ACCESO)=10
    ...
    CONSTRAINT CK_NUMERO_CELULAR CHECK (LEN(NUMERO_CELULAR) = 10)
...
CREATE TABLE BANCO(
	...
	CONSTRAINT AK_NOMBRE_BANCO UNIQUE (NOMBRE_BANCO)
);
GO
...
CREATE TABLE INTERACCION.FACTURA(
	...
	IVA			 AS			 IMPORTE_TOTAL*0.16  PERSISTED,
    ...
    CONSTRAINT FK_USUARIO_FACTURA FOREIGN KEY (ID_USUARIO) REFERENCES PERSONA.USUARIO(ID_USUARIO)
    	ON DELETE CASCADE
    	ON UPDATE CASCADE,
    CONSTRAINT CK_RFC CHECK (LEN(RFC) = 13
    AND RFC LIKE '%[A-Z]%' 
    AND RFC LIKE '%[0-9]%')
);
go
...
CREATE TABLE TELEFONO(
    ...
    CONSTRAINT CK_NUM_TELEFONO CHECK (LEN(NUM_TELEFONO)=10)
);
go
...
CREATE TABLE PERSONA.CONDUCTOR(
    ...
    FOTO_RECIENTE            varbinary(max)    NOT NULL,
...
CREATE TABLE PAGO(
    ...
    CONSTRAINT CK_ESTRUCTURA_PAGO CHECK (LEN(NUM_PAGO) = 8 AND NUM_PAGO LIKE '1%'),
);
go
...
CREATE TABLE COLOR(
    ...
    CONSTRAINT AK_NOMBRE_COLOR UNIQUE (NOMBRE_COLOR)
)
...
CREATE TABLE VEHICULO(
	...
	CONSTRAINT FK_MODELO_VEHICULO FOREIGN KEY (ID_MODELO) REFERENCES MODELO(ID_MODELO)
    	ON DELETE CASCADE
    	ON UPDATE CASCADE,
    CONSTRAINT AK_NUM_PLACA UNIQUE (NUM_PLACA),
...
CREATE TABLE QUEJA_MOTIVO(
    ...
    CONSTRAINT AK_DESCRIPCION_QUEJA_MOTIVO UNIQUE(DESCRIPCION)
)
...
CREATE TABLE RESOLUCION(
    ID_SOLUCION              int               NOT NULL        IDENTITY(1,1),
    ID_QUEJA                 int               NOT NULL,
    DESCRIPCION_RESOLUCION   varchar(100)      NOT NULL,
    FECHA_ATENDIDO           date              NOT NULL,
    CONSTRAINT PK_RESOLUCION PRIMARY KEY CLUSTERED (ID_SOLUCION),
...
CREATE TABLE MOTIVO(
    ...
    CONSTRAINT AK_NOMBRE_MOTIVO UNIQUE(NOMBRE)
);
go
...
CREATE TABLE USUARIO_DESCUENTO(
    ...
    CONSTRAINT FK_USUARIO_DESCUENTO FOREIGN KEY (ID_USUARIO) REFERENCES PERSONA.USUARIO(ID_USUARIO)
        ON DELETE CASCADE
        ON UPDATE CASCADE,
    CONSTRAINT PK_USUARIO_DESCUENTO PRIMARY KEY (ID_USUARIO ASC, ID_DESCUENTO ASC)
);
go
...
CREATE TABLE RECORRIDO.ESTATUS(
    ...
    CONSTRAINT CK_NOMBRE_ESTATUS CHECK(NOMBRE_ESTATUS IN ('S','G','C','E','B','P','A','X','T')),
    CONSTRAINT AK_NOMBRE_ESTATUS UNIQUE (NOMBRE_ESTATUS)
);
go
...
CREATE TABLE RECORRIDO.VIAJE(
    ...
    ID_FACTURA               int		       NULL,
    ID_VEHICULO              int               NOT NULL,
    CLAVE_ACTUAL             smallint          DEFAULT         'S',
    ID_USUARIO               int               NOT NULL,
    ...
    CONSTRAINT CK_TIPO_VIAJE CHECK (TIPO IN ('N','P')),
    CONSTRAINT CK_TIPO CHECK (TIPO_PAGO IN ('T','E'))
...
CREATE TABLE RECORRIDO.ESTATUS_VIAJE(
    ...
    CONSTRAINT PK_ESTATUS_VIAJE PRIMARY KEY CLUSTERED (ID_VIAJE ASC, FECHA_ESTATUS ASC)
);
go

CREATE TABLE ETIQUETA_ACEPTADO(
	ID_ETIQUETA_ACEPTADO	smallint	NOT NULL,
	DESCRIPCION				varchar(50)	NOT NULL,
...
CREATE TABLE RECORRIDO.TIPO_ACCIDENTE(
	...
	CONSTRAINT AK_TIPO_ACCIDENTE UNIQUE(DESCRIPCION)
)

CREATE TABLE DIRECCION(
	ID_DIRECCION			 int			   NOT NULL     IDENTITY(1,1),
	CALLE					 varchar(70)	   NOT NULL,
	NUM_EXTERIOR			 numeric(4,0)      NOT NULL,
...
CREATE TABLE RECORRIDO.ACCIDENTE(
    ...
    DESCRIPCION              varchar(100)      NOT NULL,
	CONSTRAINT PK_ACCIDENTE	PRIMARY KEY CLUSTERED(ID_ACCIDENTE),
    CONSTRAINT AK_ACCIDENTE_DIRECCION UNIQUE(UBICACION),
	CONSTRAINT FK_TIPO_ACCIDENTE FOREIGN KEY(ID_TIPO_ACCIDENTE) REFERENCES RECORRIDO.TIPO_ACCIDENTE(ID_TIPO_ACCIDENTE)
        ON DELETE CASCADE
        ON UPDATE CASCADE,
    CONSTRAINT FK_DIRECCION_ACCIDENTE FOREIGN KEY (UBICACION) REFERENCES DIRECCION(ID_DIRECCION)
        ON DELETE CASCADE
        ON UPDATE CASCADE
)
...
CREATE TABLE RECORRIDO.ACEPTADO(
	...
	CONSTRAINT FK_ID_ACCIDENTE FOREIGN KEY (ID_ACCIDENTE) REFERENCES RECORRIDO.ACCIDENTE(ID_ACCIDENTE)
        ON DELETE CASCADE
        ON UPDATE CASCADE,
    ...
    CONSTRAINT CK_PROPINA CHECK(PROPINA IN (0,10,15,20)),
    CONSTRAINT FK_ETIQUETA_ACEPTADO	FOREIGN KEY(ID_ETIQUETA_ACEPTADO) REFERENCES ETIQUETA_ACEPTADO(ID_ETIQUETA_ACEPTADO)
        ON DELETE CASCADE
        ON UPDATE CASCADE,
);
go
...
CREATE NONCLUSTERED UNIQUE INDEX UI_NOMBRE_USUARIO ON PERSONA.USUARIO(NOMBRE_USUARIO);


--- CODIGOS PARA REINICIAR LA BASE DE DATOS:

DROP TRIGGER PERSONA.trg_jerarquiaU
DROP trigger INTERACCION.trg_limitando_tarjeta_a_3
DROP TRIGGER PERSONA.trg_jerarquiaA
DROP TRIGGER PERSONA.trg_jerarquiaC
DROP TRIGGER trg_validar_auto
DROP TRIGGER RECORRIDO.trg_FechaSolicitud
DROP TRIGGER RECORRIDO.trg_ValidarTransicionEstatus
DROP TRIGGER RECORRIDO.trg_aceptado
...
-- PERSONA.USUARIO
DROP INDEX IX_USUARIO_ID_USUARIO_RECOMIENDA ON PERSONA.USUARIO;

-- INTERACCION.TARJETA
DROP INDEX IX_TARJETA_ID_USUARIO ON INTERACCION.TARJETA;
DROP INDEX IX_TARJETA_ID_BANCO ON INTERACCION.TARJETA;

-- INTERACCION.FACTURA
DROP INDEX IX_FACTURA_ID_USUARIO ON INTERACCION.FACTURA;

-- TELEFONO
DROP INDEX IX_TELEFONO_ID_USUARIO ON TELEFONO;

-- PAGO
DROP INDEX IX_PAGO_ID_USUARIO ON PAGO;

-- MODELO
DROP INDEX IX_MODELO_ID_MARCA ON MODELO;

-- VEHICULO
DROP INDEX IX_VEHICULO_ID_USUARIO ON VEHICULO;
DROP INDEX IX_VEHICULO_ID_MODELO ON VEHICULO;
DROP INDEX IX_VEHICULO_COLOR_VEHICULO ON VEHICULO;

-- INTERACCION.QUEJA
DROP INDEX IX_QUEJA_ID_USUARIO ON INTERACCION.QUEJA;
DROP INDEX IX_QUEJA_ID_VEHICULO ON INTERACCION.QUEJA;
DROP INDEX IX_QUEJA_ID_ADMINISTRADOR ON INTERACCION.QUEJA;
DROP INDEX IX_QUEJA_ID_MOTIVO_QUEJA ON INTERACCION.QUEJA;

-- RESOLUCION
DROP INDEX IX_RESOLUCION_ID_QUEJA ON RESOLUCION;

-- INTERACCION.DESCUENTO
DROP INDEX IX_DESCUENTO_ID_MOTIVO ON INTERACCION.DESCUENTO;

-- USUARIO_DESCUENTO
DROP INDEX IX_USUARIO_DESCUENTO_ID_USUARIO ON USUARIO_DESCUENTO;
DROP INDEX IX_USUARIO_DESCUENTO_ID_DESCUENTO ON USUARIO_DESCUENTO;

-- RECORRIDO.VIAJE
DROP INDEX IX_VIAJE_ID_FACTURA ON RECORRIDO.VIAJE;
DROP INDEX IX_VIAJE_ID_VEHICULO ON RECORRIDO.VIAJE;
DROP INDEX IX_VIAJE_CLAVE_ACTUAL ON RECORRIDO.VIAJE;
DROP INDEX IX_VIAJE_ID_USUARIO ON RECORRIDO.VIAJE;
DROP INDEX IX_VIAJE_FECHA_SOLICITUD ON RECORRIDO.VIAJE;

-- RECORRIDO.BITACORA
DROP INDEX IX_BITACORA_ID_VIAJE ON RECORRIDO.BITACORA;

-- RECORRIDO.ESTATUS_VIAJE
DROP INDEX IX_ESTATUS_VIAJE_ID_VIAJE ON RECORRIDO.ESTATUS_VIAJE;
DROP INDEX IX_ESTATUS_VIAJE_CLAVE_ESTATUS ON RECORRIDO.ESTATUS_VIAJE;

-- RECORRIDO.ACEPTADO
DROP INDEX IX_ACEPTADO_ID_VIAJE ON RECORRIDO.ACEPTADO;
DROP INDEX IX_ACEPTADO_ID_ETIQUETA_ACEPTADO ON RECORRIDO.ACEPTADO;
DROP INDEX IX_HORA_INICIO_CURSO ON RECORRIDO.ACEPTADO;

-- RECORRIDO.ACCIDENTE
DROP INDEX IX_ACCIDENTE_ID_TIPO_ACCIDENTE ON RECORRIDO.ACCIDENTE;
DROP INDEX IX_ACCIDENTE_ID_ACEPTADO ON RECORRIDO.ACCIDENTE;
DROP INDEX IX_ACCIDENTE_UBICACION ON RECORRIDO.ACCIDENTE;
...
DBCC CHECKIDENT ('QUEJA_MOTIVO', RESEED, 0);
